...
adaptive_radix_tree* new_adaptive_radix_tree()
{
  ...
  adaptive_radix_tree *art = (adaptive_radix_tree*)malloc(sizeof(adaptive_radix_tree));
  art->root = 0;
...
{
  art_node *_new = art_node_replace_leaf_child(an, key, len, off);
  if (likely(_new)) {
    if (likely(parent)) {
      if (unlikely(art_node_lock(parent))) {
        ...
        __atomic_store(ptr, &_new, __ATOMIC_RELEASE);
        art_node_unlock(parent);
        return 0;
      }
    } else { // art with only one leaf
      if (likely(__atomic_compare_exchange_n(ptr, &an, _new, 0 /* weak */, __ATOMIC_RELEASE, __ATOMIC_ACQUIRE))) {
        return 0;
      } else {
        free_art_node(_new);
        return -1;
...
static int _adaptive_radix_tree_put(art_node *parent, art_node **ptr, const void *key, size_t len, size_t off)
{
  ...
  if (p != art_node_version_get_prefix_len(v)) {
    ...
    art_node *_new = art_node_expand_and_insert(an, key, len, off, p);
    parent = art_node_get_locked_parent(an);
    art_node_set_parent_unsafe(an, _new);
    if (likely(parent)) {
      debug_assert(off);
      art_node_replace_child(parent, ((unsigned char *)key)[off - 1], an, _new);
      art_node_unlock(parent);
    } else { // this is root
      __atomic_store(ptr, &_new, __ATOMIC_RELEASE);
    }
  ...
  art_node *_new = 0;
  next = art_node_add_child(an, ((unsigned char *)key)[off], (art_node *)make_leaf(key), &_new);
  if (unlikely(_new)) {
    parent = art_node_get_locked_parent(an);
    if (likely(parent)) {
      debug_assert((int)off > p);
      art_node_replace_child(parent, ((unsigned char *)key)[off - p - 1], an, _new);
      art_node_unlock(parent);
    } else {
      __atomic_store(ptr, &_new, __ATOMIC_RELEASE);
    }
